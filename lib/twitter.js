...
Twitter.prototype.stream = function(method, params, callback) {
	...
	var url = stream_base + '/' + escape(method) + '.json?'+ querystring.stringify(params);

	var request = this.oauth.post(url,
		this.options.access_token_key,
		this.options.access_token_secret,
		querystring.stringify(params), 'text/plain');
...
Twitter.prototype.getTrendsByLocation = function(woeid, callback) {
